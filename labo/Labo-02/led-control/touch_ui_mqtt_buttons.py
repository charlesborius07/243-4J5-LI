...
class LEDControlUI:
    ...
    def _on_mqtt_message(self, client, userdata, msg):
        ...
        # Gestion des boutons physiques via MQTT
        # Si on reçoit PRESSED, on toggle la LED correspondante
        if "button/1/state" in topic and "PRESSED" in payload:
            self._toggle_led(1)
        elif "button/2/state" in topic and "PRESSED" in payload:
            self._toggle_led(2)
    ...
    def _toggle_led(self, led_num):
        """
        Toggle l'état d'une LED (1 ou 2) et publie le nouvel état via MQTT
        """
        if led_num == 1:
            self.led1_state = not self.led1_state
            state_str = "ON" if self.led1_state else "OFF"
            self._publish_mqtt(self.led1_topic, state_str)
            self.status_message = f"LED ROUGE (Local): {state_str}"
        elif led_num == 2:
            self.led2_state = not self.led2_state
            state_str = "ON" if self.led2_state else "OFF"
            self._publish_mqtt(self.led2_topic, state_str)
            self.status_message = f"LED VERTE (Local): {state_str}"
